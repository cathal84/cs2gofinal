    ...
    if request.method == 'POST' and form.validate():
        ...
        if existing_user is None:
            ...
            if form.school.data == 'not-listed':
                ...
                # create the module1, module2, module3
...
def profile():
    if session.get('email'):
        ...
        bebras1 = BebrasQuiz1.objects().filter(email=session.get('email')).first() 
        bebras2 = BebrasQuiz2.objects().filter(email=session.get('email')).first()
        return render_template('student-profile.html', user=user, assignments=assignments, bebras1=bebras1, bebras2=bebras2)
    else:
